...
class Gt3 extends Base
{
    ...
    private function validateLogin($request)
    {
        ...
        if (empty($cursor) || md5($password) != $cursor['password']) {
        ...
        $token = array(
            "id" => $cursor->user_id,
            "email" => $cursor->email,
            "authorby" => "admin@nicekun.com",
            "time" => time()
        );

        $jwt = JWT::encode($token, Config::get("JWT_PRIV_KEY"), 'RS256');
        Session::set("token", $jwt);

        if ($rememberme == 1) {
            Cookie::set("tk", $jwt);
        }
        return ResponsUtil::success("登录成功");
    }
}